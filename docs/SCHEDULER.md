...
| ~~OD client real-time self-sync~~ | ~~`backend/config/openDental.js` `startRealTimeSync()`~~ | **RESOLVED (2026-08-13) — the loop was deleted, not rehomed.** See below. | — |
| Admin status introspection | `backend/routes/admin.js` | Request-scoped but reads OD *service state* (enabled/useDatabase/testConnection), not tenant OD data; intentionally excluded from the Slice 4 reroute | Optional: route through `odAccess.getStatus(req)` / `odAccess.testConnection(req)` once admin is confirmed tenant-scoped. Low priority — no PHI. |
