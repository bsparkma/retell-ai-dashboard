...
| ~~Sync~~ | ~~`performSync`~~ | `POST /opendental/sync/trigger` | — | no | **REMOVED 2026-08-13** with the 3-minute background loop (`syncComplete` had zero listeners; ~25,000 OD calls/day). The route now runs the per-office **health probe** instead and returns the state it measured. See `backend/services/odHealthCheck.js`. |
| OD health probe | `odHealthCheck.probeOffice` | `GET/POST /opendental/sync/status`, `/sync/trigger` | **full** | no | `GET /preferences?PrefName=ProgramVersion` — verified 200 on both offices in the H0 spike (`25.4.48.0`). Read-only, carries no PHI. |
