...
import {
  Search, Phone, RefreshCw, AlertTriangle, Clock, CheckCircle2, User,
} from "lucide-react";
import { api, careInApi, type CallbackDisplay, type CareInCall } from "@/lib/api";
import { formatTimeAgo } from "@/lib/utils";
import { toast } from "sonner";
import { CallWorklist } from "./calls/CallWorklist";
...
export default function Calls() {
  ...
  // Call worklist reports its "needs attention" count so we can badge the tab.
  const [callsNeedsCount, setCallsNeedsCount] = useState(0);
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-0.5">
            Worklist, callback queue, and CareIN log
          </p>
        </div>
      </div>
      ...
      <div className="flex items-center gap-1 bg-muted rounded-lg p-1 w-fit">
        {([
          { key: "calls" as const, label: "Call Log", count: callsNeedsCount },
      ...
      {/* CALL LOG TAB — worklist */}
      {activeTab === "calls" && (
        <CallWorklist onNeedsAttentionCount={setCallsNeedsCount} />
      )}
