 ...
 *  - Copy text is signed "Generated by CareIN".
 *
...
import { addCaseEvent, getLibrary, listCases, tcErrorMessage } from "../api";
import type { TcCaseSummary, TcLibrary } from "../api";
...
export default function FloatingCalc() {
  ...
  useEffect(() => {
    ...
    getLibrary(office)
      ...
      .catch(() => {
        /* keep last-known library; lanes fall back to honest empty state */
      });
    return () => {
      cancelled = true;
    };
  ...
  const copyText =
    mode === "cob"
      ? buildCobCopyText(cobInput, cobResult)
      : buildCopyText({
          mode,
          treatmentName,
          fee,
...
function laneResultFor(
  ...
  if (p.key === "carecredit") {
    ...
    return {
      result: carecreditPromo(financedWithFee, term, ccRegularApr),
  ...
  if (p.key === "cherry") {
    ...
    return {
      result: cherry(financedWithFee, term, aprOverride),
  ...
  if (p.key === "in_house" || p.key === "in-house") {
    ...
    return {
      result: inHousePlan(financedWithFee, term, laneApr),
      sublabel: "Practice carries",
      hasFeeData: true,
    };
  }
  // Generic provider (Proceed, Sunbit, custom): amortize at its effective APR.
  // No merchant-fee table exists for these — hasFeeData:false keeps the
  // net-to-practice summary honest instead of claiming full collection.
  const laneApr = getEffectiveApr(p, term, view.overrides);
  ...
  return {
    result: {
      ...
      merchantFee: 0,
      netToPractice: financedWithFee,
      minPurchase: p.minAmount,
      warning: null,
