...
| `Test_Cigna_Downcode.edi` | Cigna Dental Health | **Downcode** — `SVC` carries a paid code (`AD:D0120`) different from the billed code (`AD:D0150`), `CO-4` |
| `Test_Bundled_Downgraded.edi` | Aetna Dental | Downgrade (`D2740` → `D2791`) *and* bundling in one claim — `CO-B15`, `CO-97`, `CO-54` |
...
| `Test_Secondary_COB.edi` | Guardian | Secondary payer: `CLP` COB sequence `11`, `AMT*D` prior payment, `OA-23` per line |
